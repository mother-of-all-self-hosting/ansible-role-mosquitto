...
#
# Tags look like `v<Mosquitto version>-<release>`, which is what this
# repository has always published (`v2.0.22-7`, `v2.1.2-3`):
#
# - if defaults/main.yml points at a Mosquitto version that has never been
#   released, the release counter restarts at 0 (`v2.1.2-0`)
# - otherwise the counter is incremented (`v2.1.2-1`), but only if something
...
# Anchored on `mosquitto_version:` so that neither the derived
# `mosquitto_container_image_tag:` nor a commented-out line can be mistaken
# for it.
version="$(sed -nE 's|^mosquitto_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Mosquitto version from $defaults_path"
	exit 1
fi

# The version value carries no leading `v` (`2.1.2`) while the tags do, and a
